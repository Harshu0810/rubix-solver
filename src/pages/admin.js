...
export async function renderAdminPage(container, router) {
  ...
  function renderLogin(errorText) {
    ...
    container.innerHTML = `
      <div class="view-container animate-fade-in" style="align-items: center; justify-content: center; min-height: 70vh;">
        <div class="card-glass" style="width: 100%; max-width: 440px; display: flex; flex-direction: column; gap: 20px; padding: 32px;">
          ...
          <button type="button" id="admin-google-btn" class="btn btn-secondary" style="width: 100%; display: flex; align-items: center; justify-content: center; gap: 10px; padding: 11px 16px; font-weight: 600; font-size: 0.95rem; border-color: rgba(255,255,255,0.16);" ${!isConfigured ? 'disabled' : ''}>
          ...
          <div style="display: flex; align-items: center; gap: 12px; margin: -2px 0;">
            ...
            <span style="font-size: 0.76rem; color: #64748B; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em;">or with credentials</span>
            <div style="flex: 1; height: 1px; background: rgba(255, 255, 255, 0.1);"></div>
          </div>
    ...
    container.querySelector('#admin-google-btn')?.addEventListener('click', async () => {
      const btn = container.querySelector('#admin-google-btn');
      btn.disabled = true;
      btn.innerHTML = `<span>Connecting to Google…</span>`;
      const res = await authService.signInWithGoogle(window.location.origin + '/admin');
      if (res && res.error) {
